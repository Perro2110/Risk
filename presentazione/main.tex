            ...
            \onslide<5->{
                \begin{tcolorbox}[
					...
					\only<6>{%
						\scriptsize
						\begin{pseudocode}
							\pckw{function} \textsc{ValeLaPena}($c$) \\
							\> $E \leftarrow \displaystyle\sum_{\text{paesi} \in c \setminus \text{miei}} \text{tr}(\text{paesi})$ \\
							\> $M \leftarrow \displaystyle\sum_{\text{paesi} \in \text{miei}} \text{tr}(\text{paesi})$ \\
							\> $\text{need} \leftarrow E - M$ \\
							\> $B \leftarrow \{\text{paesi} \in \text{miei} \mid \text{paesi} \in \partial(c)\}$ \\
							\> $\text{need} \leftarrow \text{need} - \displaystyle\sum_{\text{paesi} \in B} \text{tr}(\text{paesi})$ \\
							\> \pckw{return} $\text{need} < \text{posizionabili}(c)$ \\
							\pckw{end function}
						\end{pseudocode}
...
\end{tabular}%
			...
			\begin{column}{0.48\textwidth}
				\begin{block}{Online Boosting}
					<1->
					Addestramento \alert{incrementale} di alberi decisionali
