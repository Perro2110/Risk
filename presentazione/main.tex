...
\begin{document}
	...
	\begin{frame}{\centering Altri tentativi...}
		\begin{columns}[T]
			...
			\begin{column}{0.48\textwidth}
				\begin{block}{Addestramento incrementale}<1->
					\small
		...
		\begin{columns}[T]
			...
			\begin{column}{0.47\textwidth}
				...
				\begin{block}{Selezione della macro}
					\small
					La macro $k$ viene scelta con probabilità proporzionale
					(\alert{softmax}): pesi alti $\Rightarrow$ più probabile,
				...
				\begin{block}{Operatori genetici}
					...
					\begin{itemize}
						...
						con prob.\ 0.5.
						\item \textbf{Mutazione gaussiana}: ad ogni peso $w$ del genoma,
						con probabilità $p_m$, viene sommato un piccolo rumore casuale
						$w \mathrel{+}= \mathcal{N}(0,\sigma)$; il risultato \\ viene
						poi limitato all'intervallo $[-3, 3]$.
	...
	\begin{frame}{\centering Primi Risultati , Setup Sperimentale}
		...
		\begin{adjustbox}{width=\textwidth}
			...
			\begin{tabular}{lcccccc}
				...
				\textbf{RLGA} (Evolutivo) & \textbf{54.5\%} & \textbf{1.86} & \textbf{22.64} & \textbf{269.14} & \textbf{69.3\%}\\
				\bottomrule
			\end{tabular}
		\end{adjustbox}
		...
		\begin{columns}[T]
			...
			\begin{column}{0.48\textwidth}
				...
				\begin{alertblock}{Prossimo passo}
					\small
					Estendere la batteria di test a \alert{50+ run} per bot.\\
					Rendere possibile giocare contro i bot.\\
					Rivedere i metodi scartati, e affinare gli esistenti.
				\end{alertblock}
			\end{column}
		\end{columns}
	\end{frame}
